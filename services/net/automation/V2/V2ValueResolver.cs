...
/// <summary>
/// V2ValueResolver class, resolves an action's value from its fixed source: an analysis result
...
/// </summary>
public static class V2ValueResolver
{
    /// <summary>
    ...
    /// <returns></returns>
    public static string? Resolve(V2ValueSource? source, V2ItemScope scope, V2ContentEntry? target, V2PromptBuilder prompts)
    {
    ...
    /// </summary>
    public static string? ResolveFrom(string reference, V2ItemScope scope, V2ContentEntry? target)
    {
    ...
    /// </summary>
    public static bool? ResolveBool(string reference, V2ItemScope scope)
    {
        var parts = reference.Split('.', 2);
        if (parts.Length != 2) return null;
