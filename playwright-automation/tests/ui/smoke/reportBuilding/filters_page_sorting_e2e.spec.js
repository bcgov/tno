...
test.describe('@smoke Filters page grid column Sorting functionality', () => {
  ...
  test(`Verify sorting on Name columns on Filter page grid`, async ({}) => {
    ...
    const defaultNameColumnValuesOnUI = await gridPage.getFiltersGridNameColumnDataFromGrid(rowCount);
    ...
    const ascNameColumnValuesOnUI = await gridPage.getFiltersGridNameColumnDataFromGrid(rowCount);
    expect(await gridPage.isGridColumnSorted(ascNameColumnValuesOnUI, 'ascending')).toBe(true);

    // Sort Descending
    await gridPage.performSorting(CONSTANTS.COLUMN_NAME.NAME);
    const descNameColumnValuesOnUI = await gridPage.getFiltersGridNameColumnDataFromGrid(rowCount);
    expect(await gridPage.isGridColumnSorted(descNameColumnValuesOnUI, 'descending')).toBe(true);

    // Remove sorting
    await gridPage.performSorting(CONSTANTS.COLUMN_NAME.NAME);
    const withoutSortingNameColumnValuesOnUI = await gridPage.getFiltersGridNameColumnDataFromGrid(rowCount);
    expect(await gridPage.isGridColumnSorted(withoutSortingNameColumnValuesOnUI, 'ascending')).toBe(true);
  ...
  test(`Verify sorting on Owner columns on Report page grid`, async ({}) => {
    ...
    // Check default sorting
    const defaultOwnerColumnValuesOnUI = await gridPage.getFiltersGridOwnerColumnDataFromGrid(rowCount);
    ...
    // Remove sorting
    await gridPage.performSorting(CONSTANTS.COLUMN_NAME.OWNER);
    const withoutSortingOwnerColumnValuesOnUI = await gridPage.getFiltersGridOwnerColumnDataFromGrid(rowCount);
  ...
  test(`Verify sorting on Description columns on MMI Product page`, async ({}) => {
    ...
    // Check default sorting
    const defaultDescriptionColumnValuesOnUI = await gridPage.getFiltersGridDescriptionColumnDataFromGrid(rowCount);
    ...
    const ascDescriptionColumnValuesOnUI = await gridPage.getFiltersGridDescriptionColumnDataFromGrid(rowCount);
    expect(await gridPage.isGridColumnSorted(ascDescriptionColumnValuesOnUI, 'ascending')).toBe(true);

    // Sort Descending
    await gridPage.performSorting(CONSTANTS.COLUMN_NAME.DESCRIPTION);
    const descDescriptionColumnValuesOnUI = await gridPage.getFiltersGridDescriptionColumnDataFromGrid(rowCount);
    expect(await gridPage.isGridColumnSorted(descDescriptionColumnValuesOnUI, 'descending')).toBe(true);

    // Remove sorting
    await gridPage.performSorting(CONSTANTS.COLUMN_NAME.DESCRIPTION);
    const withoutSortingDescriptionColumnValuesOnUI = await gridPage.getFiltersGridDescriptionColumnDataFromGrid(rowCount);
