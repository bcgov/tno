...
import { type ITopicScoreRuleModel, useApiAdminTopicScoreRules } from 'tno-core';
  ...
  const controller = React.useMemo(
    () => ({
      findAllTopicScoreRules: async () => {
        const response = await dispatch<ITopicScoreRuleModel[]>(
          'find-all-topic-score-rules',
          async () => await api.findAllTopicScoreRules(),
        );
      ...
      getTopicScoreRule: async (id: number) => {
        const response = await dispatch<ITopicScoreRuleModel>(
          'get-topic-score-rule',
          async () => await api.getTopicScoreRule(id),
        );
      ...
      addTopicScoreRule: async (model: ITopicScoreRuleModel) => {
        const response = await dispatch<ITopicScoreRuleModel>(
          'add-topic-score-rule',
          async () => await api.addTopicScoreRule(model),
        );
        store.storeTopicScoreRules((rules) => {
          const items = [...rules];
      ...
      updateTopicScoreRule: async (model: ITopicScoreRuleModel) => {
        const response = await dispatch<ITopicScoreRuleModel>(
          'update-topic-score-rule',
          async () => await api.updateTopicScoreRule(model),
        );
        store.storeTopicScoreRules((rules) =>
          rules.map((t) => {
            if (t.id === response.data.id) return response.data;
      ...
      updateTopicScoreRules: async (models: ITopicScoreRuleModel[]) => {
        const response = await dispatch<ITopicScoreRuleModel[]>(
          'update-topic-score-rules',
          async () => await api.updateTopicScoreRules(models),
        );
        store.storeTopicScoreRules(response.data);
        await lookup.getLookups();
        return response.data;
      ...
      deleteTopicScoreRule: async (model: ITopicScoreRuleModel) => {
        const response = await dispatch<ITopicScoreRuleModel>(
          'delete-topic-score-rule',
          async () => await api.deleteTopicScoreRule(model),
        );
