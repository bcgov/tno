...
import { type IOrganizationModel, useApiAdminOrganizations } from 'tno-core';
  ...
  const controller = React.useMemo(
    () => ({
      findAllOrganizations: async () => {
        const response = await dispatch<IOrganizationModel[]>(
          'find-all-organizations',
          async () => await api.findAllOrganizations(),
        );
      ...
      getOrganization: async (id: number) => {
        const response = await dispatch<IOrganizationModel>(
          'get-organization',
          async () => await api.getOrganization(id),
        );
      ...
      addOrganization: async (model: IOrganizationModel) => {
        const response = await dispatch<IOrganizationModel>(
          'add-organization',
          async () => await api.addOrganization(model),
        );
      ...
      updateOrganization: async (model: IOrganizationModel) => {
        const response = await dispatch<IOrganizationModel>(
          'update-organization',
          async () => await api.updateOrganization(model),
        );
        store.storeOrganizations((organizations) =>
          organizations.map((ds) => {
            if (ds.id === response.data.id) return response.data;
      ...
      deleteOrganization: async (model: IOrganizationModel) => {
        const response = await dispatch<IOrganizationModel>(
          'delete-organization',
          async () => await api.deleteOrganization(model),
        );
        store.storeOrganizations((organizations) =>
