                        ...
                        @if (item.ContentId.HasValue)
                        {
                            ...
                            @if (item.Content.FileReferences.Count() > 0 && item.Content.FileReferences.First().ContentType.StartsWith("video/"))
                            {
                                <a class="icon video-icon" href="@($"{ViewContentUrl}{item.ContentId}")">
                                    <svg class="video-icon" xmlns="http://www.w3.org/2000/svg" height="16" width="18" viewBox="0 0 576 512">
                            ...
                            @if (item.Content.ContentType.ToString() == "AudioVideo" && item.Content.Summary.Length > 0 && item.Content.IsApproved)
                            {
                                <a class="icon" href="@($"{ViewContentUrl}{item.ContentId}")">
                                    <svg class="transcript-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
