...
test.describe('@smoke Verify filtering of headlines using Published date content filter', () => {
  test(`Editor filters headlines using Today Published date filter`, async ({}) => {
    ...
    expect(await editorHomePage.isGivenContentSearchFilterPresent(CONSTANTS.BUTTONS.TODAY)).toBe(
      true,
    );
  ...
  test(`Editor filters headlines using 24 Hours Published date filter`, async ({}) => {
    ...
    expect(
      await editorHomePage.isGivenContentSearchFilterPresent(CONSTANTS.BUTTONS.TWENTY_FOUR_HOURS),
    ).toBe(true);
  ...
  test(`Editor filters headlines using 48 Hours Published date filter`, async ({}) => {
    ...
    expect(
      await editorHomePage.isGivenContentSearchFilterPresent(CONSTANTS.BUTTONS.FOURTY_EIGHT_HOURS),
    ).toBe(true);
    await editorHomePage.selectMediaTypeFilterDailyPrint(CONSTANTS.HEADLINES.DAILY_PRINT);
  ...
  test(`Editor filters headlines using Media Type file option Dailyprint`, async ({}) => {
    ...
    const mediaTypeRowCount = await editorHomePage.getMediaTypeFromGrid(
      CONSTANTS.HEADLINES.DAILY_PRINT,
    );
  ...
  test(`Editor filters headlines using Start date and End Date filter`, async ({}) => {
    ...
    await editorHomePage.selectStartDateAndEndDateFilter(
      CONSTANTS.FILTER_DATE.START_DATE,
      CONSTANTS.FILTER_DATE.END_DATE,
    );
    await editorHomePage.hardWait(1500);
    ...
    expect(
      await editorHomePage.validateDatesInRange(
        publishedDatesOnUI,
        CONSTANTS.FILTER_DATE.START_DATE,
        CONSTANTS.FILTER_DATE.END_DATE,
      ),
  ...
  test(`Editor publishes Print content and ectracts quotes`, async ({}) => {
    const parentPage = page;
    const headlineTitle = `Automation extract Content Headline Title ${Date.now()}`;
    headlineDetailsPage = await editorHomePage.clickOnContent(CONSTANTS.HEADLINES.PRINT_CONTENT);
  ...
  test(`Verify extract queote by ownership type - `, async ({}) => {
    const parentPage = page;
    const headlineTitle = `Automation Print Content Headline Title ${Date.now()}`;
    headlineDetailsPage = await editorHomePage.clickOnContent(CONSTANTS.HEADLINES.PRINT_CONTENT);
