...
class AddMediaPage extends BasePage {
  constructor(page) {
    ...
    this.mediaTypeDropdown = page
      .locator('label:has-text("List Option")')
      .locator('..')
      .locator('.rs__input-container');

    this.listOption = (value) => page.getByText(value, { exact: true });
    ...
    this.lstOption = (value) => page.getByText(value, { exact: true });
    ...
    this.listOption2 = (value) => page.getByText(value, { exact: true });
  ...
  async enterMediaDetails(name, description, sortOrder) {
    ...
    logger.info(
      `Enter Media Tag: Name=${name}, Description=${description}, Sort Order=${sortOrder}`,
    );
  }
  async backbtnMedia() {
  ...
  async clickOnVisibleText() {
    ...
    await this.page
      .locator(`(//*[contains(@class,'ellipsis') and contains(text(),'Automation Test Name')])[1]`)
      .click();
  ...
  async enterTagDetails(code, tname, tdescription, tsortorder) {
    ...
    logger.info(
      `Enter Media Tag: Code= ${code}, Name=${tname}, Description=${tdescription}, Sort Order=${tsortorder}`,
    );
