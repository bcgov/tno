    ...
    <div style="@pageBreak">
      ...
      @if (section.Value.SectionType == ReportSectionType.Content)
      {
          ...
          int sumOfAllTopicScores = totalScoresByTopicType.Sum(s => s.Value);
              ...
              @foreach(KeyValuePair<string, Dictionary<string,int>> topicTypeGrouping in topicScoresByTopicTypeAndName )
              {
                ...
                <ul>
                  @foreach (var topicsForTopicType in topicTypeGrouping.Value)
                  {
