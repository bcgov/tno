          ...
          <div id="toc" name="toc">
            ...
            @if (section.Value.Settings.ShowHeadlines && tableSection.Value.Content.Any() && tableSection.Value.SectionType != ReportSectionType.Gallery)
            {
              <div style="margin: 10px 0 1em 24px; padding:0;">
                ...
                {
                                  /**
                * Check if content has versions and contains the current owner version.
                * If owner version exists, use its summary.
                * If no version exists, use the content default summary.
                * If content.Summary is null, use empty string as fallback.
                */

                  string summary;
                  if (content.Versions?.ContainsKey(OwnerId.Value) == true)
                  {
                      summary = content.Versions[OwnerId.Value].Summary;
                  }
                  else
                  {
