...
public static class V2ActionCatalog
{
    ...
    public static readonly IReadOnlyDictionary<string, V2ActionDescriptor> Types = new[]
    {
        ...
        new V2ActionDescriptor("content.create", "Create Content", "content", true, false, false, _process, new[]
        {
            ...
            new V2FieldSpec("copyFields", "fields", false, "Leave empty to copy the defaults: sourceId, otherSource, licenseId, mediaTypeId, publishedOn, contentType. Check 'all fields' (stored as *) to copy everything the item carries. A derived uid is always set."),
        ...
        new V2ActionDescriptor("dedupe", "Detect Duplicate", "flow", true, false, true, _process, new[]
        {
            ...
            new V2FieldSpec("remember", "bool", false, "Persist confirmed duplicates as content_link records (value 'duplicate') and skip the LLM for items already linked. Links are written on real runs only; dry runs read them."),
            new V2FieldSpec("prompt", "prompt", false, "Comparison prompt; empty uses the 'default-dedupe' library entry when present, else the built-in comparison. {content.*} and {candidate.*} tokens give a custom layout; without tokens both stories are appended automatically."),
        },
