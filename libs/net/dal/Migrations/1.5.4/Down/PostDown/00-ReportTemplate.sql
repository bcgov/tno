...
@{
  ...
  var hasAI = Sections.Any(section => section.Value.SectionType == TNO.Entities.ReportSectionType.AI);
...
@if (Content.Count() == 0 && !ViewOnWebOnly && !hasImages && !hasAI)
{
...
@if (!ViewOnWebOnly)
{
  ...
  for (var index = 0; index < Sections.Count(); index++)
  {
    ...
    if (sectionContent.Length == 0 && section.Value.Settings.HideEmpty
      ...
      && section.Value.SectionType != TNO.Entities.ReportSectionType.Data
      && section.Value.SectionType != TNO.Entities.ReportSectionType.AI) continue;
