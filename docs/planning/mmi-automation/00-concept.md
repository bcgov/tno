...
A **Step** performs the actual LLM prompt and handles the response. A step with its own filter can
...
`reportId` (run-report), optional `notificationId` (run-notification), optional `priorActionId`
(deduplicate — an earlier action in this step or a prior step), optional `objective` (score-content,
select-top), `autoExecute` ("Always run"), `isEnabled`. Each action may declare `maxCalls` to bound
how many times it can execute within a single profile run.
...
`deduplicate`), the engine skips the step's LLM call entirely, enabling pure-mechanical steps. Action ids are round-tripped by the editor so profile saves update
actions in place — required for stable `priorActionId` references.
...
| `deduplicate` | Compares the current item against each content item a configured prior action (`priorActionId` — earlier in this step or an earlier step) successfully processed this run. One LLM comparison per prior item using the action's prompt (`{content}` = current item, `{previous}` = prior item); a response containing the confirmation statement marks a duplicate and aborts the step at this position. |
...
- **Stop Remaining Actions** (`abort-step`) — position sensitive. When its confirmation statement is
  found in the LLM response, actions ordered *before* it that were confirmed are still applied, but
  no actions ordered *after* it execute for the current content item. Execution then continues with
...
- **Deduplication** (`deduplicate`) — position sensitive, and the only action type that is **not**
  ...
  the step reaches it, the engine fetches the content items the configured prior action
  (`priorActionId`) successfully processed this run and sends one comparison prompt per prior item
  (`{content}` = the current item, `{previous}` = the prior item). A response containing the
  confirmation statement marks the current item as a duplicate and aborts the step at this
  position (like `abort-step`); the run summary records a `duplicate` change with the matched
  prior content id.
...
3. Steps execute in priority order. **Filter queries execute at most once per run and are cached** —
   steps that share a filter reuse the cached results. A gating step filter
   (`applyToAutomationFilter`) processes an iterated item only when the item appears in the filter's
   results (all matches, paged the same way). An enrichment filter's results are all injected at
   `{results}`. A `start`/`end` step with **iterate over content from step filter** enabled instead
   uses its step filter's results as the iteration source: the step executes once per hit and its
   actions apply to each item (no gate, no `{results}` injection).
4. Each step execution composes one prompt (step prompt + action prompts at `{actions}`, excluding
   `deduplicate` actions) and sends it to the LLM once per content item (or once for
   `start`/`end`/`none` targets without step-filter iteration). A step with **send separate
   prompt for each action** (`sendSeparatePrompts`) instead sends one prompt per action (the step
   prompt plus that single action's prompt) at the action's position in the sequence — a
   confirmed abort stops later actions before their prompts are ever sent, and each action may
   optionally use its own LLM (`llmId` on the action) for its prompt. A step with **use chat
   completions** (`useChatCompletions`, takes precedence) instead runs as a conversation: the
   step prompt becomes the system prompt (its default excludes the News Story/Actions sections),
   and each action is its own user message (its default prompt carries the News Story and
   Actions sections) appended to a shared message history, so the model retains earlier action
   responses for consistency. Chat mode requires a deployment-based (API key) LLM; per-action
   LLM overrides still apply because the history is maintained client-side. LLM requests use a
   configurable timeout (`LLMRequestTimeoutSeconds`) and retry transient failures — timeouts,
   throttling (429), connection failures, and server errors — up to `LLMRequestAttempts`. A step
   instance that still fails is counted in the step's `failures` and the run continues with the
   next item.
