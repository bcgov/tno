...
test.describe('@smoke MMI Product column Sorting functionality', () => {
  ...
  test(`Verify sorting on Has Request columns on MMI Product page`, async ({}) => {
    ...
    // Check default sorting
    const defaultHasRequestColumnValuesOnUI =
      await gridPage.getMMIProductHasRequestColumnDataFromGrid(rowCount);
    expect(await gridPage.isGridColumnSorted(defaultHasRequestColumnValuesOnUI, 'ascending')).toBe(
      false,
    );
    expect(await gridPage.isGridColumnSorted(defaultHasRequestColumnValuesOnUI, 'descending')).toBe(
      false,
    );
    ...
    const ascHasRequestColumnValuesOnUI =
      await gridPage.getMMIProductHasRequestColumnDataFromGrid(rowCount);
    // expect(await gridPage.isGridColumnSorted(ascHasRequestColumnValuesOnUI, 'ascending')).toBe(true);

    // Sort Descending
    await gridPage.performSorting(CONSTANTS.COLUMN_NAME.HASREQUESTS);
    const descHasRequestColumnValuesOnUI =
      await gridPage.getMMIProductHasRequestColumnDataFromGrid(rowCount);
    // expect(await gridPage.isGridColumnSorted(descHasRequestColumnValuesOnUI, 'descending')).toBe(true);

    // Remove sorting
    await gridPage.performSorting(CONSTANTS.COLUMN_NAME.HASREQUESTS);
    const withoutSortingHasRequestColumnValuesOnUI =
      await gridPage.getMMIProductHasRequestColumnDataFromGrid(rowCount);
    expect(
      await gridPage.isGridColumnSorted(withoutSortingHasRequestColumnValuesOnUI, 'ascending'),
    ).toBe(false);
    expect(
      await gridPage.isGridColumnSorted(withoutSortingHasRequestColumnValuesOnUI, 'descending'),
    ).toBe(false);
  ...
  test(`Verify sorting on Description columns on MMI Product page`, async ({}) => {
    ...
    const defaultDescriptionColumnValuesOnUI =
      await gridPage.getMMIProductDescriptionColumnDataFromGrid(rowCount);
  ...
  test(`Verify sorting on Product Type columns on MMI Product page`, async ({}) => {
    ...
    // Check default sorting
    const defaultTypeColumnValuesOnUI =
      await gridPage.getMMIProductDescriptionColumnDataFromGrid(rowCount);
    expect(await gridPage.isGridColumnSorted(defaultTypeColumnValuesOnUI, 'ascending')).toBe(false);
    expect(await gridPage.isGridColumnSorted(defaultTypeColumnValuesOnUI, 'descending')).toBe(
      false,
    );
    ...
    const ascTypeColumnValuesOnUI =
      await gridPage.getMMIProductDescriptionColumnDataFromGrid(rowCount);
    expect(await gridPage.isGridColumnSorted(ascTypeColumnValuesOnUI, 'ascending')).toBe(true);

    // Sort Descending
    await gridPage.performSorting(CONSTANTS.COLUMN_NAME.TYPE);
    const descTypeColumnValuesOnUI =
      await gridPage.getMMIProductDescriptionColumnDataFromGrid(rowCount);
    expect(await gridPage.isGridColumnSorted(descTypeColumnValuesOnUI, 'descending')).toBe(true);

    // Remove sorting
    await gridPage.performSorting(CONSTANTS.COLUMN_NAME.TYPE);
    const withoutSortingTyprColumnValuesOnUI =
      await gridPage.getMMIProductDescriptionColumnDataFromGrid(rowCount);
    expect(await gridPage.isGridColumnSorted(withoutSortingTyprColumnValuesOnUI, 'ascending')).toBe(
      false,
    );
    expect(
      await gridPage.isGridColumnSorted(withoutSortingTyprColumnValuesOnUI, 'descending'),
    ).toBe(false);
